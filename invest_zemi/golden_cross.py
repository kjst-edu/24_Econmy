...
def check_golden_cross(stock_symbol):
    ...
    if last_two_days['50_MA'].iloc[-2] < last_two_days['200_MA'].iloc[-2] and \
       last_two_days['50_MA'].iloc[-1] > last_two_days['200_MA'].iloc[-1]:
        print(f"ゴールデンクロス発生: {stock_symbol}")
        return True
    else:
        print(f"ゴールデンクロスは発生していません: {stock_symbol}")
        return False

# ユーザーに銘柄番号を入力してもらう
stock_symbol = input("銘柄番号を入力してください（例: AAPL, MSFT, 7203.Tなど）: ")
